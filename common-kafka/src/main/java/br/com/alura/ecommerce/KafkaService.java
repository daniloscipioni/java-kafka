...
class KafkaService<T> implements Closeable {
            ...
            if (!records.isEmpty()) {
                ...
                for (var record : records) {
                    try {
                        ...
                        // only catcges Exceptions because no matter which Exception
                        // i want to recover and parse de next one
    ...
    private Properties getProperties(String groupId, Map<String, String> overrideProperties) {
        ...
        properties.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "172.29.112.235:9092");
        properties.setProperty(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        properties.setProperty(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, GsonDeserializer.class.getName());
        properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, groupId);
        properties.setProperty(ConsumerConfig.CLIENT_ID_CONFIG, UUID.randomUUID().toString());
        // Máximo de mensagens por vez
